...
export default function CreditLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!res.success || !res.data) {
      setLoading(false);
      setErrorMsg(res.error?.includes('401') ? 'Invalid email or password' : res.error?.includes('404') ? 'No account found' : (res.error ?? 'Login failed'));
      return;
    }
    const data = res.data;
    if (data.role !== 'BORROWER') { setLoading(false); setErrorMsg('This account is not a Borrower account.'); return; }
    ...
    <div className="min-h-screen flex bg-[#060a18]">
      ...
      <div className="hidden lg:flex lg:w-[480px] xl:w-[520px] flex-col justify-between p-10 relative overflow-hidden bg-[#0a0f1f]">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-[0.03]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="hexC" width="56" height="100" patternUnits="userSpaceOnUse" patternTransform="scale(1.2)">
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] mb-3" style={{ color: `${ACCENT}99` }}>Borrower Portal</p>
            <h1 className="text-[28px] font-semibold text-white leading-[1.2] tracking-tight">
              DeFi Credit<br />Access
            </h1>
            <p className="text-sm text-[#5a6478] leading-relaxed mt-3 max-w-[320px]">
              Borrow against your collateral, build on-chain credit, and access competitive rates across the protocol.
            </p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              { value: '4.5%', label: 'Min Rate' },
              { value: '24', label: 'Borrowers' },
            ].map((s) => (
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-[380px] space-y-8">
          ...
          <div className="text-center">
            <h2 className="text-xl font-semibold text-white tracking-tight">Borrower Sign In</h2>
            <p className="text-sm text-[#5a6478] mt-1">Access your credit dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" autoFocus className={inputCls} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="relative flex items-center">
            ...
            <span className="px-3 text-[10px] uppercase tracking-[0.15em] text-[#3a4358]">Email login</span>
            <div className="flex-1 border-t border-[#1e2540]" />
          </div>

          <p className="text-center text-xs text-[#5a6478]">
            Don&apos;t have an account?{' '}
            <Link href="/credit/signup" className="font-medium hover:underline" style={{ color: ACCENT }}>Create Borrower account &rarr;</Link>
          </p>
