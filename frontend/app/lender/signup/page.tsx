...
export default function LenderSignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    addToast({ message: `Welcome to CashNet Lending, ${data.name}!`, severity: 'success' });
    window.location.href = '/lender';
  };

  const inputCls = 'w-full h-11 px-3 text-sm bg-[#080c1a] border border-[#1e2540] rounded-lg outline-none text-white placeholder:text-[#3a4358] transition-all focus:border-[#b367ff] focus:shadow-[0_0_0_3px_rgba(179,103,255,0.1)]';
    ...
    <div className="min-h-screen flex bg-[#060a18]">
      ...
      <div className="hidden lg:flex lg:w-[480px] xl:w-[520px] flex-col justify-between p-10 relative overflow-hidden bg-[#0a0f1f]">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-[0.03]" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="hexLS" width="56" height="100" patternUnits="userSpaceOnUse" patternTransform="scale(1.2)">
              <path d="M28 2L54 18V50L28 66L2 50V18Z" fill="none" stroke="white" strokeWidth="0.5"/>
            </pattern>
          </defs>
          <rect width="100%" height="100%" fill="url(#hexLS)" />
        </svg>
        ...
        <div className="relative z-10 space-y-8">
          <div>
            <p className="text-[11px] uppercase tracking-[0.2em] mb-3" style={{ color: `${ACCENT}99` }}>Join as Lender</p>
            <h1 className="text-[28px] font-semibold text-white leading-[1.2] tracking-tight">
              Earn Yield.<br />Grow Capital.
            </h1>
            <p className="text-sm text-[#5a6478] leading-relaxed mt-3 max-w-[320px]">
              Provide liquidity to decentralized lending pools, set your rates, and earn interest on every loan.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { value: '12', label: 'Active Lenders' },
              { value: '97.8%', label: 'Repayment Rate' },
            ].map((s) => (
          ...
          <div className="space-y-3">
            {['Earn competitive APY', 'Automated liquidation protection', 'Real-time portfolio analytics'].map((f) => (
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-[380px] space-y-8">
          ...
          <div className="text-center">
            <h2 className="text-xl font-semibold text-white tracking-tight">Create Lender Account</h2>
            <p className="text-sm text-[#5a6478] mt-1">Start earning yield in minutes</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Alex Morgan" autoFocus className={inputCls} />
            </div>
            <div className="space-y-1.5">
          ...
          <div className="text-center text-xs text-[#5a6478] pt-4 border-t border-[#1e2540]">
            ...
            <Link href="/lender/login" className="font-medium hover:underline" style={{ color: ACCENT }}>Sign in &rarr;</Link>
          </div>
