...
export default function LiquidityPoolPage() {
  ...
  // MetaMask pool actions
  const {
    approveTokenA, approveTokenB, addLiquidity: addLiquidityOnChain,
    removeLiquidity: removeLiquidityOnChain, approveSwapToken, swap: swapOnChain,
    reset: resetTx, isConnected, isSigning, isConfirming, isConfirmed,
    txHash: poolTxHash, error: poolTxError,
  } = usePoolActions();
  ...
  }, [isConfirmed, fetchAll, fetchBalances]);
  ...
  // ── Add Liquidity via MetaMask ──────────────────────────────────────────
  const handleAddLiquidity = async () => {
    const amtA = parseFloat(addAmtA);
    const amtB = parseFloat(addAmtB);
    if (!amtA || amtA <= 0 || !amtB || amtB <= 0) return;
    if (!isConnected) { setAddResult('✗ Connect wallet first'); return; }
    addLiquidityOnChain(addAmtA, addAmtB);
    setAddResult(null);
  };

  const handleApproveA = () => {
    if (!addAmtA || parseFloat(addAmtA) <= 0) return;
    approveTokenA(addAmtA);
  };

  const handleApproveB = () => {
    if (!addAmtB || parseFloat(addAmtB) <= 0) return;
    approveTokenB(addAmtB);
  };

  // ── Remove Liquidity via MetaMask ───────────────────────────────────────
  const handleRemoveLiquidity = async () => {
    const shares = parseFloat(removeShares);
    if (!shares || shares <= 0) return;
    if (!isConnected) { setRemoveResult('✗ Connect wallet first'); return; }
    removeLiquidityOnChain(removeShares);
  ...
  const handleApproveSwap = () => {
    ...
    approveSwapToken(tokenIn as 'PAL' | 'BAD', swapAmount);
  };

  const handleSwap = async () => {
    const amt = parseFloat(swapAmount);
    if (!amt || amt <= 0) return;
    if (!isConnected) { setSwapError('Connect wallet first'); return; }
    const tokenIn = swapDir === 'PAL_to_BAD' ? 'PAL' : 'BAD';
    swapOnChain(tokenIn as 'PAL' | 'BAD', swapAmount);
    setSwapResult(null); setSwapError(null);
  };
  ...
  return (
    <div className="space-y-6 animate-fadeUp">
      ...
      {/* MetaMask transaction status bar */}
      {(isSigning || isConfirming || isConfirmed || poolTxError) && (
        <div className={`rounded-lg border p-3 font-mono text-xs flex items-center gap-2 ${poolTxError ? 'border-red-500/40 bg-red-500/10 text-red-400'
          ...
          {poolTxError && `❌ ${(poolTxError as any)?.shortMessage || poolTxError.message}`}
          <button onClick={resetTx} className="ml-auto text-text-tertiary hover:text-text-primary">✕</button>
        </div>
      )}
        ...
        <div className="space-y-4">
          ...
          <div className="rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-bg-secondary)] p-5">
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs font-mono text-text-tertiary">Step 1: Approve tokens → Step 2: Add Liquidity</p>
              <div className="flex gap-2">
                <button
                  onClick={handleApproveA}
                  disabled={isSigning || !addAmtA}
                  className="flex-1 py-2 font-mono text-xs rounded border border-[#00d4ff] text-[#00d4ff] hover:bg-[rgba(0,212,255,0.1)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  Approve PAL
                </button>
                <button
                  onClick={handleApproveB}
                  disabled={isSigning || !addAmtB}
                  className="flex-1 py-2 font-mono text-xs rounded border border-[#22c55e] text-[#22c55e] hover:bg-[rgba(34,197,94,0.1)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  Approve BAD
                </button>
              </div>
              <button
                ...
                disabled={isSigning || !addAmtA || !addAmtB}
              ...
              >
                {isSigning ? '⏳ MetaMask...' : '⊕ Add Liquidity'}
              </button>
              {addResult && (
                <div className={`text-xs font-mono px-3 py-2 rounded border ${addResult.startsWith('✓')
          ...
          <div className="rounded-lg border border-[color:var(--color-border)] bg-[color:var(--color-bg-secondary)] p-5">
            ...
            <div className="space-y-3">
              ...
              <button
                ...
                disabled={isSigning || !removeShares}
              ...
              >
                {isSigning ? '⏳ MetaMask...' : '⊖ Remove Liquidity'}
              </button>
              {removeResult && (
                <div className={`text-xs font-mono px-3 py-2 rounded border ${removeResult.startsWith('✓')
                  ? 'border-[#22c55e] text-[#22c55e] bg-[rgba(34,197,94,0.08)]'
                  : 'border-[#ff3860] text-[#ff3860] bg-[rgba(255,56,96,0.08)]'
                  }`}>{removeResult}</div>
              )}
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-mono rounded border transition-colors ${swapDir === 'PAL_to_BAD'
                ...
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-mono rounded border transition-colors ${swapDir === 'BAD_to_PAL'
                    ? 'border-[#00d4ff] text-[#00d4ff] bg-[rgba(0,212,255,0.1)]'
                    : 'border-[color:var(--color-border)] text-text-tertiary'
                    }`}
                >
              ...
              <p className="text-xs font-mono text-text-tertiary">Step 1: Approve token → Step 2: Swap</p>
              <div className="flex gap-2">
                <button
                  onClick={handleApproveSwap}
                  disabled={isSigning || !swapAmount}
                  className="flex-1 py-2 font-mono text-xs rounded border border-[#f0a500] text-[#f0a500] hover:bg-[rgba(240,165,0,0.1)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  Approve {inLabel}
                </button>
                <button
                  onClick={handleSwap}
                  disabled={isSigning || !swapAmount}
                  className="flex-1 py-2.5 font-mono text-sm rounded border border-[#00d4ff] text-[#00d4ff] hover:bg-[rgba(0,212,255,0.1)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                >
                  {isSigning ? '⏳ MetaMask...' : `⇄ Swap ${inLabel} → ${outLabel}`}
                </button>
              </div>
              {swapResult && (
